...
Use the JIRA REST API to fetch issue details:
```bash
JIRA_URL="${JIRA_URL:-https://redhat.atlassian.net}"
curl -s -u "${JIRA_EMAIL}:${JIRA_AUTH_TOKEN}" -H "Content-Type: application/json" "${JIRA_URL}/rest/api/2/issue/PROJECT-123"
```
...
After fetching the primary ticket, gather bounded context (1 level deep) from the ticket's relationships by querying:
- Parent ticket
- Subtasks/children
- Issue links (blocks, is blocked by, relates to, etc.)
- Web links and remote links
...
**Error handling:** If auth is missing or the primary ticket fetch fails, follow the access failure procedure above.
...
```bash
JIRA_URL="${JIRA_URL:-https://redhat.atlassian.net}"
curl -s -u "${JIRA_EMAIL}:${JIRA_AUTH_TOKEN}" -H "Content-Type: application/json" "${JIRA_URL}/rest/api/2/issue/PROJ-123?expand=renderedFields"
```

**Fetch issue with comments:**
```bash
curl -s -u "${JIRA_EMAIL}:${JIRA_AUTH_TOKEN}" -H "Content-Type: application/json" "${JIRA_URL}/rest/api/2/issue/PROJ-123?expand=renderedFields" | jq '.fields.comment.comments'
```

**Search issues by JQL:**
```bash
curl -s -u "${JIRA_EMAIL}:${JIRA_AUTH_TOKEN}" -H "Content-Type: application/json" "${JIRA_URL}/rest/api/2/search?jql=project%3DPROJ+AND+fixVersion%3D1.0.0&maxResults=50"
```
...
**GitLab MRs:**
```bash
curl -s -H "PRIVATE-TOKEN: ${GITLAB_TOKEN}" "https://gitlab.com/api/v4/projects/PROJ_ID/merge_requests/MR_IID"
```
